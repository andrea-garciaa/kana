                ...
                Some(ch) => {
                    ...
                    let kana: KanaScanState = match (accumulator.as_str(), ch) {
                        (x, 'a' | 'u' | 'i' | 'e' | 'o') if x.is_empty() => {
                            // returning NonKanaThenKana after each (.., <vowel>) without checking accumulator emptiness would be also fine, but it would incur a useless accumulator read (may be mostly free though, it was meaningful to avoid string copy before change to u8 as non-kana token)
                            match ch {
